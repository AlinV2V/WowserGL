...
import type { StudioValidator } from './validation';
...
export class EngineToolsPanel {
  ...
  constructor(
    ...
    app.store.addEventListener('selection', () => { if (this.visible && (this.tab === 'graph' || this.tab === 'frame')) this.render(); });
  ...
  private renderProfiler() {
    ...
    const history = this.profiler.history.slice(-60);
    ...
    this.overlay.innerHTML = `<div class="engine-tool-header"><strong>Renderer Profiler</strong><span>live WebGL / editor scene</span></div><div class="profiler-grid">
      ...
      ${this.metric('Materials', s.materials.toLocaleString())}
      ${this.metric('Scene Objects', s.objects.toLocaleString())}
    </div><div class="profiler-chart"><div class="chart-label">Frame time · last ${history.length} samples · max ${maxMs.toFixed(1)} ms</div><svg viewBox="0 0 100 100" preserveAspectRatio="none"><line x1="0" y1="${100 - 16.67 / maxMs * 100}" x2="100" y2="${100 - 16.67 / maxMs * 100}" class="chart-budget"/><polyline points="${points}"/></svg></div><div class="engine-note">This is the Studio renderer profiler. Switch to <strong>Game</strong> for CleanClientMMO's authoritative runtime rendering; the live bridge remains the authoring transport.</div>`;
  }
