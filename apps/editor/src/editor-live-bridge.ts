...
export const liveTargetFor = (record: EditorRecord): LiveTarget => ({
  recordId: record.id,
...
export class EditorLiveBridge extends EventTarget {
  ...
  connect() {
    ...
    url.searchParams.set('client', 'VanillaGL Studio');
    const socket = new WebSocket(url);
    this.socket = socket;
    socket.addEventListener('open', () => {
      this.setStatus('connected');
      this.log('info', `Connected to live bridge at ${this.url}`);
    });
  ...
  pushMaterial(record: EditorRecord, override: MaterialOverride) {
    return this.command({ type: 'material.set', target: liveTargetFor(record), override });
  }
  ...
  openGame(gameUrl = import.meta.env.VITE_VANILLAGL_GAME_URL ?? 'http://localhost:5173/') {
    ...
    window.open(url.toString(), 'vanillagl-live-game');
  }
  ...
  private send(packet: BridgePacket) {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      this.log('warn', 'No live bridge connection. Start `npm run bridge` or `npm run dev:studio`.');
      return false;
    }
