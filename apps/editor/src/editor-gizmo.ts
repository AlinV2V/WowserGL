...
import { applySnapshot, EditorObjectStore, isRecordLocked, snapshotTransform } from './editor-store';
...
export class EditorGizmoController extends EventTarget {
  ...
  constructor(
    ...
    this.controls.addEventListener('mouseDown', () => {
      if (this.selected && !isRecordLocked(this.selected)) this.dragStart = snapshotTransform(this.selected.object);
    });
    this.controls.addEventListener('objectChange', () => {
      if (!this.selected || isRecordLocked(this.selected)) return;
    ...
    store.addEventListener('change', () => this.refreshLockState());
    this.bindPointer();
    this.bindKeys();
  }
  ...
  setTranslationSnap(value: number | null) { this.controls.setTranslationSnap(value); }
  setRotationSnap(degrees: number | null) { this.controls.setRotationSnap(degrees === null ? null : THREE.MathUtils.degToRad(degrees)); }
  ...
  private bindKeys() {
    window.addEventListener('keydown', (event) => {
      ...
      if (event.ctrlKey || event.metaKey) {
        if (event.code === 'KeyZ') { event.preventDefault(); event.shiftKey ? this.history.redo() : this.history.undo(); return; }
        if (event.code === 'KeyY') { event.preventDefault(); this.history.redo(); return; }
        if (event.code === 'KeyD' && this.selected) {
          event.preventDefault();
          if (isRecordLocked(this.selected)) return;
          const source = this.selected;
          let copy: EditorRecord | null = null;
          this.history.execute({ label: `Duplicate ${source.model}`, redo: () => { copy = this.store.duplicate(source, this.cursorWorld?.clone()); }, undo: () => { if (copy) this.store.remove(copy); } });
          return;
        }
      }
      ...
      if ((event.code === 'Delete' || event.code === 'Backspace') && this.selected) {
        ...
        this.history.execute({ label: `Delete ${record.model}`, redo: () => this.store.remove(record), undo: () => this.store.restore(record, oldState) });
      }
  ...
  private pick(event: PointerEvent) {
    ...
    return this.raycaster.intersectObjects(this.scene.children, true).find((hit) => !this.isEditorHelper(hit.object));
  }

  private updateCursor(event: PointerEvent) {
    this.pointerFromEvent(event);
    this.raycaster.setFromCamera(this.pointer, this.cameraController.active);
    const hit = this.raycaster.intersectObjects(this.scene.children, true).find((candidate) => candidate.object.userData.editorTerrain);
  ...
  private isEditorHelper(object: THREE.Object3D) {
    for (let node: THREE.Object3D | null = object; node; node = node.parent) if (node.userData.editorNonSelectable) return true;
    return false;
  }
  ...
  private setSelection(record: EditorRecord | null) {
    ...
    if (this.highlight) { this.scene.remove(this.highlight); this.highlight.dispose(); this.highlight = null; }
    if (!record || record.state === 'deleted') return;
    if (!isRecordLocked(record)) this.controls.attach(record.object);
    this.highlight = new THREE.BoxHelper(record.object, isRecordLocked(record) ? 0x9aa0a6 : 0xffd36a);
  ...
  private refreshLockState() {
    ...
    const shouldAttach = !isRecordLocked(this.selected) && this.selected.state !== 'deleted';
    const attached = this.controls.object === this.selected.object;
    if (shouldAttach && !attached) this.controls.attach(this.selected.object);
    if (!shouldAttach && attached) this.controls.detach();
    if (this.highlight) {
      (this.highlight.material as THREE.LineBasicMaterial).color.setHex(isRecordLocked(this.selected) ? 0x9aa0a6 : 0xffd36a);
      this.highlight.update();
    }
  }

  private updateHighlight() { this.highlight?.update(); }

  private commitTransform() {
    if (!this.selected || !this.dragStart || isRecordLocked(this.selected)) { this.dragStart = null; return; }
    const record = this.selected;
    const before = this.dragStart;
    const after = snapshotTransform(record.object);
    this.dragStart = null;
    if (JSON.stringify(before) === JSON.stringify(after)) return;
    this.history.pushApplied({ label: `Transform ${record.model}`, undo: () => { applySnapshot(record.object, before); this.store.markModified(record); this.updateHighlight(); }, redo: () => { applySnapshot(record.object, after); this.store.markModified(record); this.updateHighlight(); } });
  }
}
