...
export type StudioLightPreview = {
  enabled: boolean;
  color: string;
  intensity: number;
  radius: number;
...
export type StudioBehaviorWaypoint = {
  x: number;
  y: number;
  z: number;
  waitMs?: number;
...
export type StudioBehaviorPreview = {
  ...
  leashRadius: number;
  loop: boolean;
  waypoints: StudioBehaviorWaypoint[];
};

export type StudioCharacterEquipment = {
...
export type StudioCharacterPreview = {
  race: number;
  classId: number;
  gender: number;
  skin: number;
  ...
  animationId?: number;
  scale?: number;
  equipment: StudioCharacterEquipment[];
};
