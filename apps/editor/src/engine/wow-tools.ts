...
export class WowWorldTools extends EventTarget {
  ...
  private rebuildHelpers() {
    ...
    if (light?.enabled !== false) { const radius = Math.max(0.1, Number(light.data.radius ?? 18)); const helper = new THREE.Mesh(new THREE.SphereGeometry(radius, 24, 14), new THREE.MeshBasicMaterial({ wireframe: true, transparent: true, opacity: 0.2, depthWrite: false })); record.object.getWorldPosition(helper.position); helper.userData.editorNonSelectable = true; helper.name = 'Light Radius'; this.helperRoot.add(helper); }
    const collision = this.components.getComponent(entity, 'Collision');
    if (collision?.enabled !== false && collision.data.enabled !== false && collision.data.mode !== 'none' && collision.data.debug === true) { const helper = new THREE.BoxHelper(record.object); helper.userData.editorNonSelectable = true; helper.name = 'Collision Bounds'; this.helperRoot.add(helper); }
    const portal = this.components.getComponent(entity, 'Portal');
    if (portal?.enabled !== false && portal.data.enabled !== false) { const center = new THREE.Box3().setFromObject(record.object).getCenter(new THREE.Vector3()); const helper = new THREE.ArrowHelper(new THREE.Vector3(0, 1, 0), center, 6, undefined, 1.3, 0.7); helper.userData.editorNonSelectable = true; helper.name = `Portal ${portal.data.groupA ?? 0} → ${portal.data.groupB ?? 0}`; this.helperRoot.add(helper); }
  }
